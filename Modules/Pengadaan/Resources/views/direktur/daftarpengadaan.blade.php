                ...
                <div class="card-body" style="font-size: 14px;">
                    <div class="table-responsive">
                        <table id="myTable" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach ($subPerencanaan as $item)
                                    @if ($item->pengadaan)
                                        @php
                                            $item->pengadaan->checkAndUpdateStatus();
                                        @endphp
                                    @endif
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if ($item->metodePengadaan)
                                                {{ $item->metodePengadaan->nama_metode }}
                                            @else
                                                <em>Metode belum tersedia</em>
                                            @endif
                                        </td>
                                        <td class="text-center">{{ $item->perencanaan->sumber }}</td>
                                        <td class="text-center">
                                            @php
                                                $status = 'Belum dalam periode';

                                                if ($item->pengadaan && $item->pengadaan->status) {
                                                    $status = $item->pengadaan->status->nama_status;
                                                }
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group" role="group">
                                                <button onclick="window.location.href='{{ url('/direktur/show/' . $item->id) }}'"
